...
import com.rosan.installer.ext.util.process.ProcessUtil.requireShizuku
...
object ProcessUtil {
    ...
    fun binder(clazz: Class<*>, context: Context): IBinder = ((runCatching {
        clazz.getDeclaredConstructor(Context::class.java).newInstance(context)
    }.getOrNull() ?: clazz.getDeclaredConstructor().newInstance()) as IInterface).asBinder()
    ...
    suspend fun <T> requireShizuku(action: suspend () -> T): T {
        // 检测Shizuku是否已经安装、激活
        if (!Shizuku.pingBinder()) throw ShizukuNotWorkException(SHIZUKU_NOT_WORK_EXCEPTION_MESSAGE)
        // 检测是否授权
    ...
    private suspend fun requestShizukuPermissions() = callbackFlow {
